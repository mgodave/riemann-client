 ...
 * @apiviz.landmark
 */
public class BlockingBufferedWriteHandler extends SimpleChannelHandler implements LifeCycleAwareChannelHandler {
  ...
  /**
   * Creates a new instance with the default unbounded {@link java.util.concurrent.BlockingQueue}
   * implementation and without buffer consolidation.
   */
  public BlockingBufferedWriteHandler() {
    this(false);
  }
   ...
   */
  public BlockingBufferedWriteHandler(BlockingQueue<MessageEvent> queue) {
    this(queue, false);
  }

  /**
   * Creates a new instance with {@link java.util.concurrent.ConcurrentLinkedQueue}
   *
   * @param consolidateOnFlush {@code true} if and only if the buffered write requests are merged
   *                           into a single write request on {@link #flush()}
   */
  public BlockingBufferedWriteHandler(boolean consolidateOnFlush) {
    this(new LinkedBlockingQueue<MessageEvent>(1000), consolidateOnFlush);
  }
   ...
   *                           into a single write request on {@link #flush()}
   */
  public BlockingBufferedWriteHandler(BlockingQueue<MessageEvent> queue, boolean consolidateOnFlush) {
    if (queue == null) {
   ...
   */
  protected BlockingQueue<MessageEvent> getQueue() {
    return queue;
  }
  ...
  public void flush(boolean consolidateOnFlush) {
    ...
    if (acquired = flush.compareAndSet(false, true)) {
      final BlockingQueue<MessageEvent> queue = getQueue();
